...
public class DataClient {
    ...
    public JSONObject post(String path, String data) throws Exception {
        JSONObject result;
        PostMethod method = new PostMethod(repoHostname+":"+repoHostPort+"/api/data/"+path);
        ...
        System.out.println("POST RESPONSE: "+cdata.toString());
    ...
    public JSONObject register() throws Exception {
        JSONObject result;
        PostMethod method = new PostMethod(repoHostname+":"+repoHostPort+"/api/data/store.registration");
        method.getParams().setCookiePolicy(CookiePolicy.RFC_2109);
        method.setRequestHeader("Cookie", "jsonhub-store-key="+getStoreKey());
