(ns org.composer.db.mysql.MetaManager
  (:gen-class
   :methods [
...
(defn -insert [this store-key nm link reference id]
  ...
  (catch Exception e ( jsonMsg "error" (str "failed to insert metadata for store with key " store-key)))))
...
(defn -delete [this store-key nm link reference id]
  ...
  (catch Exception e ( jsonMsg "error" (str "failed to delete metadata for store with " store-key)))))
